# 数据源架构图（V3.7.1 全量 · 更新版）

> 覆盖：a-stock-data **V3.7.1**（19 源 · 54 端点 · 11 层）+ Tushare Pro + westock-data + 问财三件套 + FTShare 系 + 新浪板块资金流 + adata
> 标注：🔴=失效 / 🟡=部分可用 / ✅=正常
...
flowchart TB
    subgraph APP[应用层]
        ...
        SK[技能层<br/>a-stock-data V3.7.1 / tushare-pro /<br/>westock-data / 问财三件套 / Superpowers]
        SCR[项目脚本 scripts/]
    end
    ...
    subgraph SOURCES[外部数据源]
        subgraph ASTOCK[a-stock-data V3.7.1 · 19 源]
...
### a-stock-data V3.7.1 主源体系（19 源 · 54 端点 · 11 层）

| 优先级 | 数据源 | 状态 | 提供 | 封IP风险 |
|---|---|---|---|---|
| 1 | 通达信 mootdx（TCP）| ✅ | K线/五档/逐笔/财务快照/F10 | 极低（不封IP）|
| 2 | 腾讯财经 | ✅ | 实时PE/PB/市值/换手/涨跌停/指数/ETF/分钟K（带 `is_stale` 僵尸报价标志）| 低 |
| 3 | 东财 datacenter | ✅ | 龙虎榜/解禁/融资融券/大宗/股东户数/分红/个股信息 | 低 |
| 4 | 东财 push2/push2his | ✅ | 板块/个股资金流（分钟级+120日）/概念/地域 | 低 |
| 5 | 问财 iwencai | ✅ | NL 语义搜研报/选股（需 Key）| 低 |
| 6 | 东财 reportapi/PDF | ✅ | 研报全文+评级+EPS+PDF 下载 | 低 |
| 7 | 同花顺热点 | ✅ | 当日强势股+题材归因 | 极低（零鉴权）|
| 8 | 同花顺 hsgtApi | ✅ | 北向资金分钟级（hgt 可用 / sgt 仅参考）| 极低（零鉴权）|
| 9 | 百度股市通 | ✅ | 概念板块+K线带MA | 极低（零鉴权）|
| 10 | 新浪财经 | ✅ | 财报三表/复权因子 qfq/hfq/期权 | 低 |
| 11 | 同花顺 basic | ✅ | 一致预期 EPS | 低 |
| 12 | 财联社 | ✅ | 全市场实时电报（官方签名零key）| 低 |
| 13 | 巨潮 cninfo | ✅ | 公告全文检索+PDF/互动易 | 低 |
| 14 | 上交所官方（备胎）| ✅ | 龙虎榜全文/实时五档（`yunhq.sse.com.cn`）| 极低（零鉴权）|
| 15 | 深交所官方（备胎）| ✅ | 龙虎榜/公告+PDF/实时五档 | 极低（零鉴权）|
| 16 | baostock（V3.7）| ✅ | 估值历史 PE/PB/PS/PCF+换手率+停牌+ST+上市退市日（**不支持北交所**）| 极低（免注册）|
| 17 | 申万研究（V3.7）| ✅ | 行业分类变迁史（公开 XLS，消除前视偏差）| 极低 |
| 18 | 人民银行（V3.7）| ✅ | 社会融资规模增量（月度）| 极低 |
| 19 | 国家统计局（V3.7）| ✅ | PMI 制造业/非制造业/综合+大中小型 | 极低 |

**11 层结构**：L1 行情 / L2 研报 / L3 信号 / L4 资金面·筹码 / L5 新闻 / L6 基础数据 / L7 公告 / L8 打板 / L9 ETF期权 / L10 舆情互动 / L11 宏观（V3.7.0 新增）
...
| 数据 | 主源 | 备胎1 | 备胎2 | 冷却/恢复 |
|---|---|---|---|---|
| 实时行情+五档 | mootdx/腾讯 | **上交所 yunhq / 深交所官方**（一手五档）| — | 恒 |
| 指数收盘 | 腾讯 | Tushare（双源交叉 <0.001 点）| — | 恒 |
| K线（全历史）| mootdx/百度/腾讯 | 同花顺 `d.10jqka.com.cn`（日/周/月/30/60分）| — | 恒 |
| K线（分钟）| mootdx | **腾讯 `ifzq.gtimg.cn` mkline**（m1-m60，唯一 5 分钟备胎）| — | 恒 |
...
脚本调用
  → market_api.api / tushare_pro_data / 工具层 / a-stock-data 技能
    → 网络层（国内数据源直连 · GitHub/境外走 clash 代理）
      → 数据源（19 源 + 配套：腾讯/东财/push2delay/同花顺/新浪/Tushare/
        HKEX/westock/adata/baostock/申万/央行/统计局/FTShare 交叉验证）
...
| 项 | 状态 |
|---|---|
| a-stock-data 版本 | ✅ **V3.7.1**（双份 md5 一致：`~/.grok/skills/` + `a-stock-data-main/`，CI 门禁）|
...
| 验证层 | 工具 |
|---|---|
| 版本保证 | `verify_a_stock_data_v360.py`（门禁：双份版本==3.7.1 + md5 + V3.7.1 API 面 + 无旧接口残留，进 CI）|
...
| 类别 | 源 | 说明 |
|---|---|---|
| 🆓 免费公开接口 | a-stock-data 19 源（除 iwencai）+ 新浪资金流 + adata + push2delay + westock + FTShare 系 | 无费用；⚠️ 风险=风控封禁/限流/接口变更/延时，靠降级链+多源交叉吸收 |
