#!/usr/bin/env python3
"""
a-stock-data V3.6.0 数据源保证门禁（本地可执行，无网络依赖）

对应 AGENTS.md「数据源版本保证（强制）」的机器可查实现，保证 AI agent 对话中
自动调用的一定是 a-stock-data V3.6.0（并覆盖 Tushare.pro / westock-data / 问财 SkillHub）。

保证点：
  G1 双份 SKILL.md 存在且版本 == 3.6.0（用户级 ~/.grok/skills/a-stock-data + 项目级 a-stock-data-main/）
  G2 双份 SKILL.md 内容一致（md5 相同）
  G3 V3.6.0 API 面完整（norm_ticker / tencent_quote(is_stale) / eastmoney_reports(老码抛错) /
...
_EXPECTED_VERSION = "3.6.0"

_IN_CI = os.environ.get("GITHUB_ACTIONS") == "true"

# V3.6.0 关键 API 面（函数定义或标志，必须在 SKILL.md 中出现）
_API_SURFACE = [
    ...
    ("def em_stock_monitor", "重点监控池（V3.6.0 新增）"),
    ("def em_price_anomaly", "日内异动池（V3.6.0 新增）"),
...
def check_g3_api_surface() -> None:
    ...
    if missing:
        ...
        _record("G3", _FAIL, f"V3.6.0 API 面缺失 {len(missing)} 项: {detail}")
    else:
        _record("G3", _PASS, f"V3.6.0 API 面完整（{len(_API_SURFACE)} 项关键函数/标志）")
...
def main() -> int:
    ...
    print("=" * 66)
    print("a-stock-data V3.6.0 数据源保证门禁" + ("（含 --live 联网冒烟）" if live else "（本地）"))
    print("=" * 66)
    n_fail = 0
    ...
    print(summary)
    if n_fail:
        print("✗ 保证门禁未通过，禁止将 a-stock-data 视为 V3.6.0 可用。")
    else:
        print("✓ 保证门禁通过：AI agent 将自动调用 a-stock-data V3.6.0（含 Tushare/westock/问财 路由）。")
