...
数据正确性修复（#37 续）。**纯修复版，无新增端点（仍为 44 端点），数据源 15 / 层数 10 不变。**

### 修复
- **腾讯市值字段纠正**：`f[44]=流通市值(亿)`，`f[45]=总市值(亿)`——旧版两字段对调。涉及 `tencent_quote()` 返回值、`full_valuation()` 的 `mcap=vals[45]`、及字段索引表。
- **920 北交所前缀路由**：`get_prefix()` 与 `tencent_quote()` 新增 `92→bj` 映射——`92xxx` 北交所新股此前被误判为沪市。
- **tencent_quote() key 碰撞修复**：显式前缀（如 `sh000001`）输入时，返回 dict key 改用原始输入（保留前缀），通过 `input_map[]` + `prefixed_to_orig` dict 防同名覆盖——旧版 strip 掉前缀后 `sh000001` 与 `sz000001` 都变成 `000001`，key 碰撞丢失数据。
- **board_fund_flow() 分页补全**：旧版仅请求第一页 200 条，行业/概念板块实际 > 200 个时被截断——新增 `_paginate()` 自动翻页取全量（最多 10 页 × 200）。
- **tdx_client(market=...) 验活跳过**：非 `std` 市场（如 `bj`/`sh`）走 `_validate()` 拉 `000001` 上证 K 线必然失败误抛异常——改为仅 `market='std'` 时验活，非 std 市场仅 TCP 探测。
- **东财文档补充**：风控阈值补 `45000次/小时` 硬限制 + 建议 `em_get()` 内部 `EM_MIN_INTERVAL` ≥ 2s。

### 涉及文件
- `scripts/tencent_api.py` — 市值字段 swap、920 前缀、key 碰撞修复
- `scripts/valuation.py` — mcap 字段纠正 + 前缀修复
- `scripts/data_gate.py` / `scripts/mean_reversion_exec.py` — 920 前缀补充
- `scripts/eastmoney_api.py` — board_fund_flow 分页
- `scripts/mootdx_api.py` — tdx_client 非 std 验活跳过
- `docs/SKILL.md` / `a-stock-data-main/SKILL.md` / `.codebuddy/skills/a-stock-data/SKILL.md` — 文档同步
