...
> **V3.5.1（数据正确性修复，2026-07-28 · #37 续）：**
> - **纯修复版，无新增端点（仍为 44 端点）**；所有修复均为数据正确性与接口健壮性。
> - **腾讯市值字段纠正**：`f[44]` 为**流通市值**(float_mcap_yi)，`f[45]` 为**总市值**(mcap_yi)，旧版两字段对调。涉及 `tencent_quote()` 返回值、`full_valuation()` 的 `mcap=vals[45]`、及 §1.2 字段索引表。
> - **920 北交所前缀**：`get_prefix()` 与 `tencent_quote()` 新增 `5`/`8`/`4`/`92` 北交所前缀识别，旧版 `92xxx` 被误判为沪市。
> - **tencent_quote() key 冲突**：显式前缀（如 `sh000001`）输入返回 dict key 改用原始输入（保留前缀），旧版 strip 掉前缀后 `sh000001` 与 `sz000001` 都变成 `000001`导致 key 碰撞丢失数据。
> - **board_fund_flow() 分页补全**：旧版仅请求第一页 200 条，行业/概念板块实际 > 200 个时被截断；新增 `_paginate()` 自动翻页取全量。
> - **tdx_client(market=...) 验活跳过**：非 `std` 市场（如 `bj`/`sh`）走 `_validate()` 拉 `000001` 上证 K 线必然失败误抛异常；改为仅 `market='std'` 时验活。
> - **东财文档补充**：风控阈值补 `45000次/小时` 硬限制 + 建议 `em_get()` 内部 `EM_MIN_INTERVAL` ≥ 2s。
>
...
2. **每次间隔 ≥ 1 秒 + 随机抖动**（QPS ≤ 2），批量筛选时调大到 **1.5~2 秒**（45000 次/小时 = 12.5 QPS 硬限，安全余量下建议 ≤ 2 QPS）
3. **复用 HTTP 会话**（Keep-Alive），不要每次新建连接
4. **带正常 UA + Referer**（本 SKILL 各端点已配好）
5. **批量场景每只股票之间 sleep**——AI 跑批量循环（如筛选 100 只股逐个拉龙虎榜/资金流）是被封的头号元凶
6. **小心 45000 次/小时硬封禁**——超出后 IP 级封禁持续 2-24 小时，`em_get()` 默认 1s 间隔上限即 3600 次/小时，安全；若调小间隔须注意不要触碰此限

### 已内置限流：所有东财请求走 `em_get()`

本 SKILL 提供统一的节流入口 `em_get()`（定义见下方「东财数据中心统一查询（共用 helper）」），它自动做到：串行限流（最小间隔 `EM_MIN_INTERVAL=1.0s` + 随机抖动）+ 复用 `EM_SESSION`（Keep-Alive）+ 默认 UA。**所有 `eastmoney.com` 端点的代码块都已改用 `em_get` 而非裸 `requests.get`**，AI 直接抄代码即自带防封。批量任务把 `EM_MIN_INTERVAL` 调大即可进一步降速（注：1s 间隔 = 3600 次/小时，远低于 45000 硬限；调小间隔前务必确认不撞墙）。
...
def tdx_client(market='std'):
    ...
    V3.5.1 修复：仅 market='std' 时验活——_validate() 内部拉 000001（上证）K 线，
    非 std 市场（bj/sh 等）拉上证 K 线必然失败，旧版会误抛异常。非 std 市场仅 TCP 探测。
    """
    should_validate = market == 'std'   # V3.5.1: 非 std 市场无条件跳过验活
    for ip, port in _TDX_SERVERS:
        if not _probe(ip, port):
            continue
        try:
            c = Quotes.factory(market=market, server=(ip, port))
            if not should_validate or _validate(c):
                return c
        except Exception:
...
def get_prefix(code: str) -> str:
    ...
    if c.startswith(("5", "6", "9")):        # 5x=沪 ETF/LOF，6/9=沪个股
        return "sh"
    if c.startswith(("4", "8", "92")):       # 4x/8x/92x=北交所（920 新增代码段，V3.5.1 修复）
        return "bj"
...
> **歧义说明：** `000001` 默认按个股→`sz000001`（平安银行）；要上证指数请显式传 `sh000001`。`000016` 默认按沪指数→上证50；要深康佳A 请传 `sz000016`。**V3.5.1 修复：** `92xxx` 北交所新股代码段已加入 `get_prefix()`，旧版会被误判为沪市。北交所现有三段：`4x`/`8x`/`92x`。
...
def tencent_quote(codes: list[str]) -> dict[str, dict]:
    ...
    prefixed = []
    input_map = []   # (original_code, prefixed_code_with_prefix)  V3.5.1: 保留原始输入做 key，防显式前缀碰撞
    for c in codes:
        low = c.lower()
        ...
        elif c.startswith(("4", "8", "92")):          # V3.5.1: 92x 北交所新股代码段
    ...
    # 构建 prefixed → result 的索引，用原始输入 code 做 key（V3.5.1 防 key 碰撞）
    prefixed_to_orig = {p.lstrip("shszbj"): orig for orig, p in input_map}
    result = {}
    for line in data.strip().split(";"):
        if not line.strip() or "=" not in line or '"' not in line:
            continue
        key = line.split("=")[0].split("_")[-1]     # e.g. "sh688017" or "bj920123"
        vals = line.split('"')[1].split("~")
        if len(vals) < 53:
            continue
        code = key.lstrip("shszbj")                  # 纯 6 位数字
        orig_code = prefixed_to_orig.get(code, code) # V3.5.1: 找回原始输入
        result[orig_code] = {
            "name":         vals[1],
            ...
            "mcap_yi":      float(vals[45]) if vals[45] else 0,   # V3.5.1 修复: f[45]=总市值(亿)
            "float_mcap_yi":float(vals[44]) if vals[44] else 0,   # V3.5.1 修复: f[44]=流通市值(亿)
...
def board_fund_flow(board_type: str = "industry", period: str = "today",
    ...
    # V3.5.1: 自动翻页取全量（行业/概念板块 > 200 个，单页截断数据不完整）
    def _paginate():
        all_items = []
        for pn in range(1, 10):   # 最多 10 页 × 200 = 2000，足够覆盖
            params = {**base_params, "pn": str(pn)}
            r = em_get(url, params=params, headers={"User-Agent": UA}, timeout=15)
            page_items = r.json().get("data", {}).get("diff", []) or []
            if not page_items:
                break
            all_items.extend(page_items)
            if len(page_items) < 200:  # 最后一页不足一页
                break
        return all_items
...
def full_valuation(code: str) -> dict:
    """单票完整估值分析"""
    # 1. 腾讯实时行情
    prefix = "sh" if code.startswith(("6","9")) else ("bj" if code.startswith(("4","8","92")) else "sz")   # V3.5.1: 92x=北交所新股
    ...
    price = float(vals[3])
    mcap = float(vals[45])   # V3.5.1 修复: f[45]=总市值(亿), f[44]=流通市值
    pe_ttm = float(vals[39]) if vals[39] else 0
    pb = float(vals[46]) if vals[46] else 0
