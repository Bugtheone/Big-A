...
> **V3.5.1 数据正确性修复（2026-07-24）：** 纯质量修复版，无新端点（仍为44端点）。修复7项：① 腾讯市值字段纠正——`f[44]=流通市值(亿)`、`f[45]=总市值(亿)`（旧版对调）；② 920开头北交所新股前缀路由——`get_prefix()` 与 `tencent_quote()` 补充 `92→bj` 映射；③ `tencent_quote()` key碰撞修复——显式前缀输入用 `input_map[]`+`prefixed_to_orig` 防同名覆盖；④ `board_fund_flow()` 行业/概念板块自动翻页——最多10页×200条，避免单页截断；⑤ `tdx_client()` 非std市场跳过验活——非std市场仅TCP探测不走真实取数验活；⑥ 东财45000/小时硬限风控补充；⑦ 版本号/杂项更新。
> >
...
> **V3.5.1 — data correctness fix (2026-07-24):** Pure quality fix, no new endpoints (still 44). 7 fixes: ① Tencent mkt-cap field swap — `f[44]=float mkt cap (亿)`, `f[45]=total mkt cap (亿)` (old version swapped them); ② 920-prefix BSE new stocks — `get_prefix()` & `tencent_quote()` now map `92→bj`; ③ `tencent_quote()` key collision fix — explicit prefix input routed via `input_map[]`+`prefixed_to_orig` dict; ④ `board_fund_flow()` auto-pagination for industry/concept — up to 10 pages × 200; ⑤ `tdx_client()` skip validation for non-std markets — TCP-probe only, no real-fetch validation; ⑥ Eastmoney 45k/hour hard rate-limit supplement; ⑦ version number & misc updates.
> >
...
```
China A-Share Full-Stack Data · 10-Layer Architecture · V3.5.1
